...
class Element:
    def __init__(self=None, symbol=None, name=None, mass=None, number=None, cross_section=None):
        self.symbol = symbol
        self.name = name
        self.mass = mass
        self.number = number
...
def generate_atoms():
    # Start by creating an empty dictionary for the elements
    atom = {}
    # Iterate through the atomic_number dictionary to create elements
    for item in atom_data:
        # Create the Element object
        element = Element(symbol=item[1],
                       name = item[2],
                          mass=item[3], 
                          number=item[0], 
                          cross_section=cross_section.get(item[1], None))
        # Add isotopes
        if item[1] in isotope_data.keys() and isotope_data[item[1]]!=None:
            for iso in isotope_data[item[1]]:
                mass_number, isotope_mass, abundance = iso
                #isotope_cross_section = cross_section.get(symbol, None)  # You can use the same cross_section for isotopes
                # Create the Isotope object and append to the element
                element.isotope.append(Isotope(mass=isotope_mass, 
                                               abundance=abundance, 
...
# Port all element data to a new python file
def write_elements(dict_of_elements:dict, filename='exported_elements.py'):
...
# Generate the elements dictionary
elements = generate_atoms()
# Call the function to write to file
write_elements(elements)

print("exported_atom_data has been generated successfully!")
